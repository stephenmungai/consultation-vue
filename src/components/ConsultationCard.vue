<template>
    <div class="consultation-card">
        <div class="doctor-profile row align-center">
            <img :src="profileImage" alt="">
            <p class="doctor-name">{{ profile['first_name'] }} {{ profile['last_name'] }}</p>
        </div>
        <p class="doctor-number">{{ profile['phone_number'] }}</p>
        <p class="doctor-specs" v-if="!isDoctor">{{ profile['specialization']['name'] }}</p>
        <p class="disease">{{ disease }}</p>
        <button class="outline-btn" @click="consult" v-if="!diagnosed">Message</button>
        <button class="outline-btn" @click="prescription" v-if="diagnosed">
...
export default {
    ...
    computed: {
        ...
        profile() {
            return this.user['is_doctor'] ?
                ...
                this.$props.data['ticket']['application']['patient']
                : this.$props.data['ticket']['application']['doctor']
        },
        isDoctor() {
            return this.user['is_doctor']
        },
        disease() {
            return this.$props.data['diagnosis'] == "" ? "Not Specified" : this.$props.data['diagnosis']
        },
        diagnosed(){
            if(this.$props.data['prescription']!=null && this.$props.data['prescription']!=""  ){
                return true
            }
            return false
        },
    ...
    methods: {
        consult() {
            this.$router.push(`consultations/${this.$props.data['ticket']['id']}`)
        },
        prescription(){
            this.$router.push(`prescription/${this.$props.data['ticket']['id']}`)
        }
    },
