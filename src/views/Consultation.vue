<template>
    <DiagnosisForm @close='openModal=false' v-if="openModal" />
    <div class="consultation-page">
        <MediaPanel />
        <section id="messages-section">
            <div class="messages-header" v-if="consultation!=null">
                <img :src="profileImage"
                    onerror="this.src='https://images.unsplash.com/photo-1544005313-94ddf0286df2?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=688&q=80'"
                    alt="" class="profile-logo">
                <p class="doctor-name section-title">{{profile['first_name']}} {{profile['last_name']}}</p>
                <div class="row" id="make-diag" @click="openModal=true" v-if="user['is_doctor']">
                    <img src="../assets/icons/stethoscope-svgrepo-com.svg" alt="">
                    <h5> Make Diagnosis
                    </h5>
                </div>
            </div>
            <div class="messages-main">

                <div class="messages-list">
                    <Message v-for='(message,index) in messages' :key="index" :message="message" />
                    <div ref="scrollable"></div>
                </div>
                <div class="message-input">
                    <input type="text" placeholder="Type something ..." v-model="message"
                        v-on:keyup.enter="sendMessage">
                    <button id="send-msg-btn" @click="sendMessage">Send</button>
                </div>
            </div>
        </section>
    </div>
</template>
<script>
import Message from '../components/Consultation/Message.vue';
import DiagnosisForm from '../components/Consultation/DiagnosisForm.vue';
import firebase from 'firebase';
import MediaPanel from '../components/Consultation/MediaPanel.vue'
// import { mapGetters } from 'vuex';
export default {
    components: {
        Message, DiagnosisForm, MediaPanel
    },
    data() {
        return {
            consultation: null,
            openModal: false,
            db: firebase.firestore(),
            messages: [],
            message: '',
            user: {}
        }
    },
    computed: {
        // ...mapGetters({ user: 'getUser' }),
        profile() {
            return this.user['is_doctor'] ? this.consultation['ticket']['application']['patient'] : this.consultation['ticket']['application']['doctor']
        },
        profileImage() {


            if (this.profile['image'] == null) {
                return "data:image/png;base64,"

            }
            else {
                return this.profile['image']
            }
        },
    },
    methods: {
        async sendMessage() {
            const messageInfo = {
                "user": this.user['id'],
                "text": this.message,
                "createdAt": Date.now()
            }
            await this.db.collection(`messages/ticket/${this.$route.params.id}`).add(messageInfo)
            this.message = ''
            this.$refs['scrollable'].scrollIntoView({ behaviour: 'smooth' })
        }
    },

    async mounted() {
        this.user = await this.$store.dispatch('populateUser')
        var detailUrl = this.user['is_doctor'] ? "doctors/consultation/" : 'patients/consultation/'
        var resTicket = await this.$axios.get(`${detailUrl}${this.$route.params.id}`)
        if (resTicket.status <= 210) {
            this.consultation = resTicket.data;
        }

        this.db.collection(`messages/ticket/${this.$route.params.id}`).orderBy('createdAt').onSnapshot(querySnap => {
            this.messages = querySnap.docs.map(doc => doc.data())
        })
        this.$refs['scrollable'].scrollIntoView({ behaviour: 'smooth' })

    }

}
</script>
<style>
@import url("../styles/consultation.css");
</style>