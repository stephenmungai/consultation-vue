<template>
    <div class="download-btn icon-container" @click="printDownload">
        <i class="fa fa-download"></i>
    </div>
    <div class="prescription" ref="DownloadComp" v-if="prescription">
        <div class="prescription-header">

            <div class="logo">
                <img src="../assets/logo.png" alt="">
                <div class="col">
                    <h3>Consult</h3>
                    <p class="logo-tagline">Online Consultation Services</p>
                </div>
            </div>

            <div class="consult-details">
                <p>123 ABC Place</p>
                <p>info@consult.com</p>
                <p>0722976321</p>
            </div>
        </div>
        <section class="prescription-date">

            <p><span>Started On:</span> {{formatDate}}</p>

        </section>
        <section class="prescription-doctor">
            <p class="prescription-title">Doctor</p>
            <div class="row">
                <img :src="profileImage" alt="" class="profile-photo">
                <div class="col">
                    <p>{{doctor['first_name']}} {{doctor['last_name']}}</p>
                    <p>{{doctor['specialization']['name']}}</p>
                </div>
            </div>
        </section>

        <section class="prescription-remarks">
            <p class="prescription-title">Remarks</p>
            <p id="remarks">{{prescription['remarks']}}</p>
        </section>

        <section class="prescription-actual">

            <p class="prescription-title">Your Prescription</p>
            <p class="medicine">{{prescription['prescription']}}</p>
        </section>

        <section class="prescription-footer">
            <p>We are happy to serve you</p>
        </section>
    </div>
</template>

<script>
import html2pdf from "html2pdf.js";
import moment from 'moment'

export default {
    props: ['consultation'],
    data(){
        return {
            prescription:null
        }
    },
    computed:{
        doctor(){
            return this.prescription['ticket']['application']['doctor']
        },
        profileImage() {
            if(this.doctor.image==null){
                return "data:image/png;base64,"

            }
            else{
                return this.doctor['image']
            }
        },
        formatDate() {
            return moment(this.prescription.date).format("MMM Do Y ");

        },
    },
    methods: {
        printDownload() {

            const invoice = document.querySelector('.prescription');
            var opt = {
                margin: 1,
                filename: "prescription",
            };
            html2pdf().from(invoice).set(opt).save();
        }
    },

    async mounted() {
        var res = await this.$axios.get(`doctors/prescription/${this.$route.params.id}`)
        this.prescription = res.data
    }
}

</script>

<style>
@import url('../styles/prescription.css');
</style>